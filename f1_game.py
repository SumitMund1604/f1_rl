"""
F1 Racing Game - Core game logic and Pygame rendering

This module handles:
- Track generation with curves and obstacles
- Car physics (position, velocity, rotation)
- Collision detection
...
@dataclass
class Checkpoint:
    """Track checkpoint for progress tracking"""
    x: float 
    y: float
    radius: float = 40.0
    passed: bool = False
...
class F1Game:
    """
    F1 Racing Game with track, obstacles, and car physics.
    
    The track is an oval/circuit shape with obstacles to avoid.
    """
    ...
    def __init__(self, width: int = 800, height: int = 600):
        ...
        self.track_width = 80  # Width of the road
        ...
        # Sensor configuration (5 sensors for wall detection)
        self.num_sensors = 5
    ...
    def _setup_track(self):
        """Initialize checkpoints and obstacles"""
        # Create checkpoints around the oval track
        num_checkpoints = 8
        self.checkpoints = []
        for i in range(num_checkpoints):
            angle = 2 * math.pi * i / num_checkpoints
            # Place checkpoints on the center of the track
    ...
    def _generate_obstacles(self):
        """Generate random obstacles on the track"""
        self.obstacles = []
        num_obstacles = 6
        
        for i in range(num_obstacles):
            angle = 2 * math.pi * (i + 0.5) / num_obstacles  # offset from checkpoints
    ...
    def reset(self) -> np.ndarray:
        ...
        # Start car at the right side of the track, facing up
    ...
    def step(self, action: int) -> Tuple[float, bool, dict]:
        ...
        elif action == 2:  # Turn right + accelerate
            self.car.angle += self.car.turn_speed
            self.car.speed = min(self.car.speed + self.car.acceleration * 0.5, self.car.max_speed)
        elif action == 3:  # Brake
        ...
        if dist_to_cp < 100:
            reward += 1  # Getting close to checkpoint
        ...
        info = {
            'score': self.lap_count,
            'checkpoints': self.current_checkpoint,
            'steps': self.steps
        }
    ...
    def _check_wall_collision(self) -> bool:
        """Check if car hit the track walls"""
        # Calculate car position relative to track center
        ...
        # Car is outside outer wall or inside inner wall
    ...
    def _check_checkpoint(self) -> bool:
        """Check if car passed through current checkpoint"""
    ...
    def get_state(self) -> np.ndarray:
        ...
        State vector (15 dimensions):
            ...
            [5-6]:   Car velocity components (vx, vy) normalized
            [7]:     Car angle normalized (-1 to 1)
            [8-9]:   Direction to next checkpoint (normalized)
            [10-14]: 5 obstacle proximity sensors (normalized 0-1)
        """
        state = np.zeros(15, dtype=np.float32)
        
        # Wall distance sensors (5 sensors)
        for i, angle_offset in enumerate(self.sensor_angles):
            ...
            state[i] = dist / self.sensor_length  # Normalize
        ...
        # Obstacle proximity sensors (5 sensors)
        for i, angle_offset in enumerate(self.sensor_angles):
            sensor_angle = self.car.angle + math.radians(angle_offset)
            dist = self._cast_ray_to_obstacle(sensor_angle)
            state[10 + i] = dist / self.sensor_length  # Normalize
    ...
    def _cast_ray_to_obstacle(self, angle: float) -> float:
        ...
        for obs in self.obstacles:
            # Calculate distance along the ray to the obstacle
            dx = obs.x - self.car.x
            dy = obs.y - self.car.y
    ...
    def render(self, mode: str = "human") -> Optional[np.ndarray]:
        ...
        # Draw track (outer and inner ellipses)
        # Outer boundary
        ...
        if self.car:
            ...
            # Draw sensors (optional, for debugging)
    ...
    def render_with_fps(self, fps: int = 60):
        """Render with frame rate control"""
        self.render()
        if self.clock:
            self.clock.tick(fps)
